...
class StockPrice {
 private:
  ...
  struct min_stock_info {
    ...
    bool operator<(const min_stock_info& b) const { return price > b.price; }
  };
  struct max_stock_info {
    int timestamp, price;
    max_stock_info(int t, int p) : timestamp(t), price(p) {}
    bool operator<(const max_stock_info& b) const { return price < b.price; }
  };
 ...
 public:
  ...
  int current() { return current_price_; }
...
class StockPrice2 {
 ...
 public:
  ...
  int maximum() { return *prices_set_.rbegin(); }

  int minimum() { return *prices_set_.begin(); }
};
...
TEST(test2034, SAMPLE1) {
  ...
  EXPECT_EQ(stockPrice.current(),
            5);  // 返回 5 ，最新时间戳为 2 ，对应价格为 5 。
  EXPECT_EQ(stockPrice.maximum(),
            10);  // 返回 10 ，最高价格的时间戳为 1 ，价格为 10 。
  stockPrice.update(1, 3);  // 之前时间戳为 1 的价格错误，价格更新为 3 。
      // 时间戳为 [1,2] ，对应股票价格为 [3,5] 。
  EXPECT_EQ(stockPrice.maximum(), 5);  // 返回 5 ，更正后最高价格为 5 。
  stockPrice.update(4, 2);  // 时间戳为 [1,2,4] ，对应价格为 [3,5,2] 。
  EXPECT_EQ(stockPrice.minimum(),
            2);  // 返回 2 ，最低价格时间戳为 4 ，价格为 2 。
}
