...
class Solution {
 public:
  vector<int> topStudents(vector<string>& positive_feedback,
    ...
    ranges::transform(
        report, student_id, score.begin(),
        [](long long sc, unsigned id) { return -(sc << 32 | id); },
        [&](auto&& s) {
          istringstream ss(s);
          return accumulate(istream_iterator<string>(ss), {}, 0l,
                            [&](auto a, auto&& s) {
                              return a + pos.count(s) * 3 - neg.count(s);
                            });
        },
        negate{});
...
TEST(test2512, SAMPLE1) {
  ...
  int k = 2;
  vector ans{1, 2};
  EXPECT_EQ(s.topStudents(positive_feedback, negative_feedback, report,
                          student_id, k),
            ans);
}
...
TEST(test2512, SAMPLE3) {
  ...
  istringstream ss(s);
  std::for_each(istream_iterator<string>(ss), {},
                [](auto& s) { cout << s << endl; });
}
