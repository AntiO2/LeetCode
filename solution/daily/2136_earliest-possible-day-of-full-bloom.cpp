...
#include <numeric>
/*
 * 1.   可以证明，开始种一株花，最好连续种完。假设i在j之前种完，那么j完全生长的时间是固定的(plant i+ plant j + grow j)
 *      对于i来说，如果种植时没有穿插j的种植，才能尽可能早的开花。
 * 2.   假设前面的种植已经完成，现在还剩i和j两株花，且i比j先种。那么从这刻开始理论上的完全开花时间是max(plant i + grow i, plant i + plant j + grow j)
 *
 * 3.   若i和j可以交换，说明交换后结果更优。 max(plant i + grow i, plant i + plant j + grow j) > max(p j + g j , p j + p i + g i)
 *      pi + max(g i,pj + g j) > pj + max(gj, pi + g i)
 *
 ...
 *      可以说此时先种谁都无所谓。（因为按照第一点，最后的生长时间总是plant i+ plant j + grow i/j）
 */
class Solution {
public:
    int earliestFullBloom(std::vector<int> &plantTime, std::vector<int> &growTime) {
        ...
        std::sort(index.begin(), index.end(), [&growTime](const int &a, const int &b) {
