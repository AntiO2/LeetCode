 ...
 public:
  vector<int> filterRestaurants(vector<vector<int>>& restaurants,
    ...
    auto end = std::partition(
        restaurants.begin(), restaurants.end(), [&](const auto& item) {
          return (item[2] || !veganFriendly) && (item[3] <= maxPrice) &&
                 item[4] <= maxDistance;
        });
    std::sort(restaurants.begin(), end,
...
TEST(test1333, SAMPLE1) {
  Solution s;
  auto r = vector<vector<int>>{{1, 4, 1, 40, 10},
                               {2, 8, 0, 50, 5},
                               {3, 8, 1, 30, 4},
                               {4, 10, 0, 10, 3},
                               {5, 1, 1, 15, 1}};
